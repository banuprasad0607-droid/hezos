...
async function run() {
  try {
    ...
    // Bulk insert schools
    const schoolInsertValues = schoolIds.map((id, idx) => `('${id}', 'Simulated School ${idx + 1}', '00000000-0000-0000-0000-${String(idx).padStart(12, '0')}')`).join(',\n');
    await client.query(`
    ...
    const adminUserIds = schoolIds.map((_, idx) => `e${String(idx).padStart(7, '0')}-1000-0000-0000-000000000000`);
    
    const profileInsertValues = schoolIds.map((schoolId, idx) => {
    ...
    const roleInsertValues = schoolIds.map((schoolId, idx) => {
      const adminId = adminUserIds[idx];
    ...
    for (let s = 0; s < schoolCount; s++) {
      ...
      for (let stud = 0; stud < studentsPerSchool; stud++) {
        ...
        studentInsertRows.push(`('${studentId}', '${schoolId}', 'Simulated Student ${s + 1}-${stud + 1}')`);
      }
    }
