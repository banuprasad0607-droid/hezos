...
async function run() {
  ...
  try {
    ...
    const sqlPath = path.join(
      ...
      "supabase",
      "migrations",
      "20260609110000_achievements_and_awards.sql",
    );
    ...
    const attendanceStats = [1.0, 0.98, 0.9, 0.85, 0.95, 0.7, 0.88, 0.99];
    const dates = [];
    for (let i = 0; i < 30; i++) {
      const d = new Date();
    ...
    const extraAwards = [
      {
        index: 0,
        cat: "subject_topper",
        title: "Mathematics Topper Award",
        desc: "Achieved the highest score in Mathematics across all examinations in Class 1A.",
        cert: "Subject Topper Certificate",
        certNo: "HZ-2526-SUB1",
      },
      {
        index: 1,
        cat: "attendance_topper",
        title: "Perfect Attendance Champion",
        desc: "Maintained an outstanding 100% attendance track record during the academic session.",
        cert: "Best Attendance Certificate",
        certNo: "HZ-2526-ATT1",
      },
      {
        index: 7,
        cat: "discipline_award",
        title: "Best Discipline & Leadership Award",
        desc: "Recognized for exemplary behavior, leadership qualities, and adherence to school values.",
        cert: "Best Discipline Certificate",
        certNo: "HZ-2526-DIS1",
      },
    ];
    ...
    const parentNotifications = [
      {
        studentIdx: 0,
        title: "Rank #1 Achieved — Aarav Sharma",
        body: "Congratulations! Your child Aarav Sharma has achieved Rank 1 in Class 1A for the academic year 2025-2026. Academic Excellence certificate has been generated.",
        type: "rank",
      },
      {
        studentIdx: 0,
        title: "Achievement Poster Available — Aarav Sharma",
        body: "A premium achievement poster has been generated for Aarav Sharma. You can view, customize themes, and download it from the parent dashboard.",
        type: "poster",
      },
      {
        studentIdx: 1,
        title: "Attendance Champion Award — Vihaan Patel",
        body: "Congratulations! Vihaan Patel was awarded the Perfect Attendance Champion in Class 1A for the term. Download the certificate in your dashboard.",
        type: "award",
      },
    ];
    ...
    for (const pn of parentNotifications) {
      ...
      const award = awardsSeed.find(
        (a) => a.student_id === sId && a.cat.startsWith(pn.type === "rank" ? "rank" : ""),
      );
      await client.query(
        `
      ...
      `,
        [schoolId, parentUserId, sId, award ? award.award_id : null, pn.type, pn.title, pn.body],
      );
    }
