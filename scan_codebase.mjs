    ...
    if (line.includes("supabase.from(") && !line.includes("super_admin")) {
      ...
      if (match) {
        ...
        if (tenantTables.includes(table)) {
          ...
          const hasFilter =
            queryBlock.includes("school_id") ||
            queryBlock.includes("student_id") ||
            queryBlock.includes("class_id") ||
            queryBlock.includes("user_id") ||
            queryBlock.includes("teacher_id") ||
            queryBlock.includes("parent_user_id") ||
            queryBlock.includes("current_school_id") ||
            queryBlock.includes("auth.uid()");
          if (!hasFilter) {
...
if (report.hardcodedBrandingIssues.length === 0) {
  report.details.push({
    ...
    status: "PASS",
    info: "No stray HEZO SCHOOL branding in tenant-scoped screens.",
  });
} else {
  report.details.push({
    check: "Branding isolation",
    status: "FAIL",
    info: `${report.hardcodedBrandingIssues.length} branding leaks found!`,
  });
}

if (report.queryIsolationIssues.length === 0) {
  report.details.push({
    check: "Tenant query isolation check",
    status: "PASS",
    info: "All verified client queries contain school-scoping filters.",
  });
} else {
  ...
  report.details.push({
    check: "Tenant query isolation check",
    status: "WARNING",
    info: `${report.queryIsolationIssues.length} queries lack explicit school filters, relying solely on Row Level Security (RLS).`,
  });
}
