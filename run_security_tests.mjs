...
async function run() {
  try {
    ...
    if (adminASchools.rowCount === 1 && adminASchools.rows[0].id === schoolAId) {
      report.details.push({ check: "Admin A School isolation check", status: "PASS", info: "Admin A only sees School A" });
    } else {
      report.status = "FAIL";
      report.details.push({ check: "Admin A School isolation check", status: "FAIL", info: `Admin A saw: ${JSON.stringify(adminASchools.rows)}` });
    }
    ...
    if (adminAStudents.rowCount === 1 && adminAStudents.rows[0].id === studentAId) {
      report.details.push({ check: "Admin A Student isolation check", status: "PASS", info: "Admin A only sees Student A1" });
    } else {
      report.status = "FAIL";
      report.details.push({ check: "Admin A Student isolation check", status: "FAIL", info: `Admin A saw: ${JSON.stringify(adminAStudents.rows)}` });
    }
    ...
    if (adminBSchools.rowCount === 1 && adminBSchools.rows[0].id === schoolBId &&
        adminBStudents.rowCount === 1 && adminBStudents.rows[0].id === studentBId) {
      report.details.push({ check: "Admin B Isolation checks", status: "PASS", info: "Admin B only sees School B and Student B1" });
    } else {
      report.status = "FAIL";
      report.details.push({ check: "Admin B Isolation checks", status: "FAIL", info: "Admin B saw other schools or other students" });
    }
    ...
    if (teacherAStudents.rowCount === 1 && teacherAStudents.rows[0].id === studentAId &&
        teacherAClasses.rowCount === 1 && teacherAClasses.rows[0].id === classAId) {
      report.details.push({ check: "Teacher A Isolation checks", status: "PASS", info: "Teacher A only sees School A students/classes" });
    } else {
      report.status = "FAIL";
      report.details.push({ check: "Teacher A Isolation checks", status: "FAIL", info: "Teacher A saw other school students or classes" });
    }
    ...
    if (parentAStudents.rowCount === 1 && parentAStudents.rows[0].id === studentAId) {
      report.details.push({ check: "Parent A Student isolation check", status: "PASS", info: "Parent A only sees own child (Student A1)" });
    } else {
      report.status = "FAIL";
      report.details.push({ check: "Parent A Student isolation check", status: "FAIL", info: `Parent A saw: ${JSON.stringify(parentAStudents.rows)}` });
    }
    ...
    try {
      ...
      report.status = "FAIL";
      report.details.push({ check: "Privilege Escalation: Teacher write school_credentials", status: "FAIL", info: "Teacher successfully wrote to school_credentials!" });
    } catch (e) {
      report.details.push({ check: "Privilege Escalation: Teacher write school_credentials", status: "PASS", info: "Teacher write blocked as expected: " + e.message });
    }
