...
async function run() {
  try {
    ...
    if (missingSchoolsCols.length === 0) {
      report.details.push({ check: "schools table columns", status: "PASS", info: "All columns present: " + requiredSchoolsCols.join(", ") });
    } else {
      report.status = "FAIL";
      report.details.push({ check: "schools table columns", status: "FAIL", info: "Missing columns: " + missingSchoolsCols.join(", ") });
    }
    ...
    if (missingUsersCols.length === 0) {
      report.details.push({ check: "users view/table columns", status: "PASS", info: "All columns present: " + requiredUsersCols.join(", ") });
    } else {
      report.status = "FAIL";
      report.details.push({ check: "users view/table columns", status: "FAIL", info: "Missing columns: " + missingUsersCols.join(", ") });
    }

    // 3. Verify school_id exists in critical tables
    const criticalTables = [
      'students', 'classes', 'attendance', 'homework', 'remarks', 'announcements', 
      'exams', 'mark_entries', 'awards', 'certificates', 'posters', 'visitor_passes', 
    ...
    for (const table of criticalTables) {
      ...
      if (colCheck.rowCount > 0) {
        report.details.push({ check: `school_id in ${table}`, status: "PASS", info: "school_id exists" });
      } else {
        report.status = "FAIL";
        report.details.push({ check: `school_id in ${table}`, status: "FAIL", info: "school_id column is missing!" });
      }
    }
    ...
    for (const table of criticalTables) {
      ...
      if (count === 0) {
        report.details.push({ check: `orphan records in ${table}`, status: "PASS", info: "0 orphan records" });
      } else {
        report.status = "FAIL";
        report.details.push({ check: `orphan records in ${table}`, status: "FAIL", info: `${count} orphan records found!` });
      }
    }
    ...
    if (schoolNameIssues === 0 && schoolNameCheck.rowCount > 0) {
      report.details.push({ check: "school_name correct value check", status: "PASS", info: "school_name is generated successfully and matches 'name' column" });
    } else if (schoolNameCheck.rowCount === 0) {
      report.details.push({ check: "school_name correct value check", status: "PASS", info: "No schools in DB to verify, schema check is passed" });
    } else {
      report.status = "FAIL";
      report.details.push({ check: "school_name correct value check", status: "FAIL", info: `Found issues with school_name generated values in ${schoolNameIssues} rows.` });
    }
