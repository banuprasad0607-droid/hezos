...
async function run() {
  ...
  try {
    ...
    const sqlPath = path.join(
      ...
      "supabase",
      "migrations",
      "20260609130000_id_cards_module.sql",
    );
    ...
    const studentsRes = await client.query(
      "SELECT id, full_name FROM public.students WHERE school_id = $1",
      [schoolId],
    );
    ...
    for (let i = 0; i < staffRolesRes.rows.length; i++) {
      ...
      const designation =
        staff.user_id === adminUserId ? "School Admin" : designations[i % designations.length];
      const department =
        staff.user_id === adminUserId ? "Administration" : departments[i % departments.length];
      const blood = bloodGroups[i % bloodGroups.length];
    ...
    const visitors = [
      {
        ...
        host: "Senior Teacher (Mathematics)",
        passNo: "VP-2026-0001",
        checkOut: null,
      },
      {
        name: "Dr. Priya Murthy",
        purpose: "Guest Lecture in Science",
