...
import { Plus, Users, Mail, Phone, ExternalLink, Camera, User, UserCheck, ShieldAlert, Download, Trash } from "lucide-react";
...
function KpiWidget({ title, count, subtext, icon }: { title: string; count: number; subtext: string; icon: React.ReactNode }) {
  return (
...
interface Klass { id: string; name: string; grade?: string | null; section?: string | null }
interface Student {
  id: string;
  full_name: string;
...
function StudentsPage() {
  ...
  const load = async () => {
    ...
    try {
      let query = supabase
        .from("students")
        .select("id, full_name, roll_number, admission_number, photo_url, parent_email, parent_phone, parent_name, parent_user_id, class_id, classes(name)", { count: "exact" })
        .eq("school_id", effectiveSchoolId)
        .is("deleted_at", null);
      ...
      setStudents(
        ((data ?? []) as unknown as Array<Student & { classes: { name: string } | { name: string }[] | null }>).map((s) => ({
          ...s,
  ...
  useEffect(() => {
    if (!effectiveSchoolId) return;
    supabase.from("classes").select("id, name, grade, section").eq("school_id", effectiveSchoolId).is("deleted_at", null).order("name").then(({ data }) => {
      setClasses(data ?? []);
  ...
  const handleCSVImport = async (e: FormEvent) => {
    ...
    try {
      ...
      if (batchStudents.length === 0) {
        throw new Error("No valid student rows found in the CSV. Make sure you have a 'name' or 'fullname' header column.");
      }
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      setOpen(false);
      setFullName(""); setAdmissionNumber(""); setRollNumber("");
      setParentName(""); setParentEmail(""); setParentPhone(""); setParentPassword("");
      setCroppedPhoto(null); setCropTarget(null);
      void load();
      void loadKPIs();
    } catch (err: any) {
  ...
  const handleExport = async (format: "csv" | "excel" | "pdf") => {
    ...
    const headers = ["Admission No", "Student Name", "Roll No", "Parent Name", "Parent Email", "Parent Phone"];
    ...
    else if (format === "pdf") exportToPDF("Student_Roster", "Enrolled Students Directory", headers, rows);
    toast.success("Download started!");
  };

  const handleSoftDelete = async (id: string, name: string) => {
    if (!effectiveSchoolId) return;
    if (!confirm(`Are you sure you want to delete ${name}? This will move them to the Recycle Bin.`)) return;
  ...
  return (
    ...
    <>
      <PageHeader
        ...
        actions={
          <>
            ...
            />
            <button onClick={() => setBulkImportOpen(true)} className="px-3 py-1.5 text-xs font-semibold border border-border bg-card text-card-foreground rounded-md inline-flex items-center gap-1 hover:bg-secondary cursor-pointer">
              Import CSV
            </button>
            <button onClick={() => setBulkPhotoOpen(true)} className="px-3 py-1.5 text-xs font-semibold border border-border bg-card text-card-foreground rounded-md inline-flex items-center gap-1 hover:bg-secondary cursor-pointer">
              Upload Photos
            </button>
            <button onClick={() => setOpen(true)} disabled={classes.length === 0} className="px-4 py-1.5 text-sm font-medium bg-primary text-primary-foreground rounded-md inline-flex items-center gap-1 disabled:opacity-50 cursor-pointer">
            ...
            <div className="flex gap-1 border border-border rounded-md overflow-hidden bg-card text-card-foreground shrink-0 shadow-xs">
              <button onClick={() => handleExport("csv")} className="p-1.5 hover:bg-secondary transition-colors" title="Export CSV"><Download className="size-4 text-muted-foreground" /></button>
              <button onClick={() => handleExport("excel")} className="p-1.5 hover:bg-secondary border-l border-border transition-colors text-xs font-bold" title="Export Excel">XLS</button>
              <button onClick={() => handleExport("pdf")} className="p-1.5 hover:bg-secondary border-l border-border transition-colors text-xs font-bold" title="Export PDF">PDF</button>
            </div>
          </>
        }
      />
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-6 bg-background text-foreground">
        ...
        {classes.length > 0 && (
          <div className="mb-5">
            <p className="text-[10px] uppercase tracking-wider font-semibold text-muted-foreground mb-2">Filter by class</p>
            <div className="flex flex-wrap gap-2">
              <button
                onClick={() => { setFilterClassId("all"); setPage(0); }}
              ...
              {classes.map((c) => (
                <button
                  key={c.id}
                  onClick={() => { setFilterClassId(c.id); setPage(0); }}
                  className={`px-3 py-1.5 text-xs font-medium rounded-full border transition-colors cursor-pointer ${
                    filterClassId === c.id
                      ? "bg-primary text-primary-foreground border-primary"
                      : "bg-card text-card-foreground border-border hover:bg-secondary"
        ...
        {busy && students.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground text-sm">Loading records from database...</div>
        ) : students.length === 0 ? (
          <div className="bg-card border border-dashed border-border rounded-xl p-16 text-center text-card-foreground">
            ...
            <h3 className="font-semibold mt-3">{totalEnrolled === 0 ? "No students yet" : "No matches"}</h3>
            <p className="text-sm text-muted-foreground mt-1">{classes.length === 0 ? "Create a class first." : "Add your first student."}</p>
          </div>
        ) : (
          <div className="bg-card border border-border rounded-xl overflow-hidden shadow-xs text-card-foreground">
                ...
                {students.map((s) => {
                  ...
                  return (
                    <tr key={s.id} className="hover:bg-secondary/40 transition-colors">
                      <td className="px-6 py-3 font-mono text-xs text-foreground">{s.admission_number ?? "—"}</td>
                      <td className="px-6 py-3 font-medium text-foreground">
                        <Link to="/students/$studentId" params={{ studentId: s.id }} className="hover:text-brand flex items-center gap-3">
                          <div className="size-8 rounded-full border border-border bg-slate-100 dark:bg-slate-800 overflow-hidden flex items-center justify-center shrink-0">
                            {s.photo_url ? (
                              <img src={s.photo_url} alt="" className="size-full object-cover" loading="lazy" />
                            ) : (
                      ...
                      <td className="px-6 py-3">
                        <div className="space-y-0.5">
                          {s.parent_name && <p className="text-xs font-medium text-foreground">{s.parent_name}</p>}
                          {s.parent_email && (
                            <p className="inline-flex items-center gap-1 text-xs text-muted-foreground">
                              <Mail className="size-3" /> {s.parent_email}
                              {s.parent_user_id ? (
                                <span className="ml-1 text-[10px] bg-success-soft text-success px-1.5 py-0.5 rounded font-semibold">Linked</span>
                              ) : (
                                <span className="ml-1 text-[10px] bg-warning-soft text-warning px-1.5 py-0.5 rounded font-semibold">Pending</span>
                              )}
                            </p>
                          )}
                          {s.parent_phone && (
                          ...
                          {!s.parent_email && !s.parent_phone && <span className="text-xs text-muted-foreground">—</span>}
                        </div>
                      </td>
                      <td className="px-6 py-3 text-right">
                        <div className="flex justify-end gap-3 items-center">
                          {wa && (
                            <a href={wa} target="_blank" rel="noreferrer" className="text-xs font-medium text-[#25D366] inline-flex items-center gap-1 hover:underline" title="WhatsApp parent">
                              WhatsApp
                            </a>
                          )}
                          <Link to="/students/$studentId" params={{ studentId: s.id }} className="text-xs font-medium text-brand inline-flex items-center gap-1 hover:underline">
            ...
            <div className="flex items-center justify-between p-4 bg-secondary/10 border-t border-border">
              <p className="text-xs text-muted-foreground">
                Showing {totalCount > 0 ? page * pageSize + 1 : 0} to {Math.min((page + 1) * pageSize, totalCount)} of {totalCount} records
              </p>
      ...
      {open && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50" onClick={() => setOpen(false)}>
          <form onClick={(e) => e.stopPropagation()} onSubmit={submit} className="bg-card text-card-foreground rounded-xl p-6 w-full max-w-xl space-y-4 shadow-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex flex-col gap-2 p-3 border border-border rounded-lg bg-secondary/20">
              <span className="text-xs font-semibold text-muted-foreground uppercase">Student Photo</span>
              <div className="flex items-center gap-4">
                ...
                <div className="flex-grow">
                  ...
                  <p className="text-[10px] text-muted-foreground mt-1">Supports JPG, PNG, WEBP. Will be cropped automatically.</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                ...
                <input required value={fullName} onChange={(e) => setFullName(e.target.value)} className="mt-1 w-full px-3 py-2 text-sm border border-border rounded-md bg-background text-foreground" />
              </div>
              <div>
                <label className="text-sm font-medium">Admission # *</label>
                <input required value={admissionNumber} onChange={(e) => setAdmissionNumber(e.target.value)} className="mt-1 w-full px-3 py-2 text-sm font-mono border border-border rounded-md bg-background text-foreground" />
              </div>
              <div>
                <label className="text-sm font-medium">Roll #</label>
                <input value={rollNumber} onChange={(e) => setRollNumber(e.target.value)} className="mt-1 w-full px-3 py-2 text-sm border border-border rounded-md bg-background text-foreground" />
              </div>
              <div className="col-span-2">
                <label className="text-sm font-medium">Class *</label>
                <select required value={classId} onChange={(e) => setClassId(e.target.value)} className="mt-1 w-full px-3 py-2 text-sm border border-border rounded-md bg-background text-foreground">
                  {classes.map((c) => {
                    const suffix = [c.grade, c.section].filter(Boolean).join(" · ");
                    return <option key={c.id} value={c.id}>{c.name}{suffix ? ` — ${suffix}` : ""}</option>;
                  })}
                </select>
              </div>
            </div>
            ...
            <div className="pt-2 border-t border-border">
              <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">Parent / Guardian</p>
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  <label className="text-sm font-medium">Parent name</label>
                  <input value={parentName} onChange={(e) => setParentName(e.target.value)} className="mt-1 w-full px-3 py-2 text-sm border border-border rounded-md bg-background text-foreground" />
                </div>
                <div>
                  <label className="text-sm font-medium">Parent email</label>
                  <input type="email" value={parentEmail} onChange={(e) => setParentEmail(e.target.value)} placeholder="parent@example.com" className="mt-1 w-full px-3 py-2 text-sm border border-border rounded-md bg-background text-foreground" />
                </div>
                <div>
                  <label className="text-sm font-medium">Parent WhatsApp #</label>
                  <input value={parentPhone} onChange={(e) => setParentPhone(e.target.value)} placeholder="+919876543210" className="mt-1 w-full px-3 py-2 text-sm border border-border rounded-md bg-background text-foreground" />
                </div>
                <div className="col-span-2">
            ...
            <div className="flex gap-2 justify-end pt-2">
              <button type="button" onClick={() => { setOpen(false); setCroppedPhoto(null); setCropTarget(null); }} className="px-4 py-2 text-sm font-medium border border-border rounded-md hover:bg-secondary cursor-pointer">Cancel</button>
              <button type="submit" disabled={busy} className="px-4 py-2 text-sm font-semibold bg-primary text-primary-foreground rounded-md disabled:opacity-50 cursor-pointer">{busy ? "Adding…" : "Add Student"}</button>
            </div>
          </form>
        </div>
      )}
      ...
      {cropTarget && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-[60]" onClick={(e) => e.stopPropagation()}>
          <div className="bg-card p-6 rounded-xl w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
      ...
      {bulkImportOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50" onClick={() => setBulkImportOpen(false)}>
          <form onClick={(e) => e.stopPropagation()} onSubmit={handleCSVImport} className="bg-card text-card-foreground rounded-xl p-6 w-full max-w-md space-y-4 shadow-lg">
            <h2 className="font-semibold text-lg">Bulk Student Import (CSV)</h2>
            <p className="text-xs text-muted-foreground">
              Import multiple students instantly. Your CSV should have columns like: <strong>name, admission_no, roll_no, class_name, parent_name, parent_email, parent_phone</strong>.
            </p>
            <div>
            ...
            <div className="flex gap-2 justify-end pt-2">
              <button type="button" onClick={() => { setBulkImportOpen(false); setCsvFile(null); }} className="px-4 py-2 text-sm font-medium border border-border rounded-md hover:bg-secondary cursor-pointer">Cancel</button>
              <button type="submit" disabled={csvUploading} className="px-4 py-2 text-sm font-semibold bg-brand text-brand-foreground rounded-md disabled:opacity-50 cursor-pointer">
      ...
      {bulkPhotoOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50" onClick={() => setBulkPhotoOpen(false)}>
          <form onClick={(e) => e.stopPropagation()} onSubmit={handleBulkPhotoUpload} className="bg-card text-card-foreground rounded-xl p-6 w-full max-w-md space-y-4 shadow-lg">
            <h2 className="font-semibold text-lg">Bulk Photo Sync Upload</h2>
            <p className="text-xs text-muted-foreground">
              Select multiple student photo files. The filename (e.g. <code>2026-0001.jpg</code> or <code>Aarav Sharma.png</code>) must match the student's **admission number**, **roll number**, or **full name** case-insensitively.
            </p>
            <div>
            ...
            <div className="flex gap-2 justify-end pt-2">
              <button type="button" onClick={() => { setBulkPhotoOpen(false); setPhotoFiles(null); }} className="px-4 py-2 text-sm font-medium border border-border rounded-md hover:bg-secondary cursor-pointer">Cancel</button>
              <button type="submit" disabled={photoUploading || !photoFiles} className="px-4 py-2 text-sm font-semibold bg-brand text-brand-foreground rounded-md disabled:opacity-50 cursor-pointer">
