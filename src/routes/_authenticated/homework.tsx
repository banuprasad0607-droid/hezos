...
function HomeworkPage() {
  ...
  const load = async () => {
    ...
    const { data } = await supabase
      .from("homework")
      .select("id, class_id, title, subject, description, due_date, file_url, file_type, created_at, classes(name)")
      .eq("school_id", effectiveSchoolId)
      .order("created_at", { ascending: false });
    setItems(
      ((data ?? []) as unknown as Array<Homework & { classes: { name: string } | { name: string }[] | null }>).map((d) => ({
        ...d,
  ...
  useEffect(() => {
    if (!effectiveSchoolId) return;
    supabase.from("classes").select("id, name").eq("school_id", effectiveSchoolId).order("name").then(({ data }) => {
      setClasses(data ?? []);
  ...
  const recipientsFor = (cid: string): BroadcastRecipient[] =>
    parents.filter((p) => p.class_id === cid).map((p) => ({
      id: p.id,
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      setOpen(false);
      setTitle(""); setSubject(""); setDescription(""); setDueDate(""); setFile(null);
      void load();
    } catch (err) {
  ...
  return (
    ...
    <>
      <PageHeader
        ...
        actions={
          <button onClick={() => setOpen(true)} disabled={classes.length === 0} className="px-4 py-1.5 text-sm font-medium bg-primary text-primary-foreground rounded-md inline-flex items-center gap-1 disabled:opacity-50">
      ...
      <div className="flex-1 overflow-y-auto p-8">
        {items.length === 0 ? (
          <div className="bg-card border border-dashed border-border rounded-xl p-16 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">{classes.length === 0 ? "Create a class first." : "Post your first assignment."}</p>
          </div>
        ) : (
            ...
            {items.map((h) => (
              <article key={h.id} className="bg-card border border-border rounded-xl p-5">
                <div className="flex items-start justify-between gap-3">
                  ...
                  {h.file_type !== "none" && (
                    <span className="shrink-0 inline-flex items-center gap-1 text-xs bg-brand-soft text-brand px-2 py-1 rounded">
                      {h.file_type === "pdf" ? <FileText className="size-3" /> : <ImageIcon className="size-3" />}
                      {h.file_type.toUpperCase()}
                    </span>
                  )}
                </div>
                {h.description && <p className="text-sm text-muted-foreground mt-2 line-clamp-3">{h.description}</p>}
                <div className="flex items-center justify-between mt-4">
                  ...
                  {h.file_url && (
                    <a href={h.file_url} target="_blank" rel="noreferrer" className="text-xs text-brand font-medium inline-flex items-center gap-1 hover:underline">
      ...
      {open && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50" onClick={() => setOpen(false)}>
          <form onClick={(e) => e.stopPropagation()} onSubmit={submit} className="bg-card rounded-xl p-6 w-full max-w-lg space-y-4 shadow-lg max-h-[90vh] overflow-y-auto">
            <h2 className="font-semibold text-lg">Create Homework</h2>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-sm font-medium">Class *</label>
                <select required value={classId} onChange={(e) => setClassId(e.target.value)} className="mt-1 w-full px-3 py-2 text-sm border border-border rounded-md bg-background">
                  {classes.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
                </select>
              </div>
              <div>
                <label className="text-sm font-medium">Subject</label>
                <input value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="Mathematics" className="mt-1 w-full px-3 py-2 text-sm border border-border rounded-md bg-background" />
              </div>
            </div>
            <div>
              <label className="text-sm font-medium">Title *</label>
              <input required value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Algebraic Expressions Ex 4.2" className="mt-1 w-full px-3 py-2 text-sm border border-border rounded-md bg-background" />
            </div>
            <div>
              <label className="text-sm font-medium">Description</label>
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="mt-1 w-full px-3 py-2 text-sm border border-border rounded-md bg-background" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-sm font-medium">Due date</label>
                <input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} className="mt-1 w-full px-3 py-2 text-sm border border-border rounded-md bg-background" />
              </div>
              <div>
                <label className="text-sm font-medium">Attach (PDF or image)</label>
                <input type="file" accept=".pdf,image/*" onChange={(e) => setFile(e.target.files?.[0] ?? null)} className="mt-1 w-full text-sm" />
              </div>
            </div>
            <div className="flex gap-2 justify-end pt-2">
              ...
              <button type="submit" disabled={busy} className="px-4 py-2 text-sm font-semibold bg-primary text-primary-foreground rounded-md disabled:opacity-50">{busy ? "Posting…" : "Post Homework"}</button>
            </div>
          </form>
        </div>
      )}
