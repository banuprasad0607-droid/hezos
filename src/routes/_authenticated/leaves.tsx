      ...
      <PageHeader
        ...
        actions={
          isParent ? (
            <button
              onClick={() => {
                setForm((f) => ({ ...f, student_id: children[0]?.id ?? "" }));
                setOpen(true);
              }}
            ...
            {leaves.map((l) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  {l.review_note && (
                    <p className="text-xs text-muted-foreground mt-1">Note: {l.review_note}</p>
                  )}
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  {isParent && l.status === "pending" && (
                    <button
                      onClick={() => cancel(l.id)}
                      className="text-xs text-muted-foreground border border-border px-3 py-1.5 rounded-md hover:bg-accent"
                    >
                      Cancel
                    </button>
                  )}
                  {isStaff && l.status === "pending" && (
                    <>
                      <button
                        ...
                        className="text-xs font-medium bg-success-soft text-success px-3 py-1.5 rounded-md inline-flex items-center gap-1"
                      >
                        <Check className="size-3" /> Approve
                      </button>
                      <button
                        onClick={() => review(l.id, "rejected")}
                        className="text-xs font-medium bg-danger-soft text-danger px-3 py-1.5 rounded-md inline-flex items-center gap-1"
                      >
