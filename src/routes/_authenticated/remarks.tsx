...
function RemarksPage() {
  ...
  const load = async () => {
    ...
    setFetching(true);
    try {
      let query = (supabase as any).from("remarks")
        .select("id, student_id, category, content, created_at, visible_to_parent, students!inner(full_name, parent_name, parent_phone)", { count: "exact" })
  ...
  useEffect(() => {
    if (!effectiveSchoolId) return;
    supabase.from("students").select("id, full_name").eq("school_id", effectiveSchoolId).is("deleted_at", null).order("full_name").then(({ data }) => {
      setStudents(data ?? []);
  ...
  const handleSoftDelete = async (id: string) => {
    if (!effectiveSchoolId) return;
    if (!confirm("Are you sure you want to delete this remark? This will move it to the Recycle Bin.")) return;
    try {
      const { error } = await (supabase as any).from("remarks")
  ...
  const handleExport = async (format: "csv" | "excel" | "pdf") => {
    ...
    try {
      const { data, error } = await (supabase as any).from("remarks")
  ...
  return (
    ...
    <>
      <PageHeader
        ...
        actions={
          <div className="flex gap-2">
            {canManage && (
              <button onClick={() => setOpen(true)} disabled={students.length === 0} className="px-4 py-1.5 text-sm font-medium bg-primary text-primary-foreground rounded-md inline-flex items-center gap-1 disabled:opacity-50 cursor-pointer">
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-6 bg-background text-foreground">
        ...
        <div className="bg-card border border-border rounded-xl p-4 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex gap-1 border border-border rounded-md overflow-hidden bg-background">
            <button onClick={() => handleExport("csv")} className="p-1.5 hover:bg-secondary text-[10px] font-bold px-2 border-r border-border transition-colors cursor-pointer text-foreground">CSV</button>
            <button onClick={() => handleExport("excel")} className="p-1.5 hover:bg-secondary text-[10px] font-bold px-2 border-r border-border transition-colors cursor-pointer text-foreground">XLS</button>
            <button onClick={() => handleExport("pdf")} className="p-1.5 hover:bg-secondary text-[10px] font-bold px-2 transition-colors cursor-pointer text-foreground">PDF</button>
          </div>
        </div>

        {fetching && remarks.length === 0 ? (
          <div className="text-center py-12 text-sm text-muted-foreground">Loading remarks ledger...</div>
        ) : remarks.length === 0 ? (
          <div className="bg-card border border-dashed border-border rounded-xl p-16 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">{students.length === 0 ? "Add a student first." : "Start with an appreciation note."}</p>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="bg-card border border-border rounded-xl divide-y divide-border shadow-xs text-card-foreground">
              {remarks.map((r) => {
                ...
                return (
                  <div key={r.id} className="p-5 flex gap-4 hover:bg-secondary/10 transition-colors">
                    <div className={`size-10 shrink-0 rounded-lg flex items-center justify-center text-xs font-bold ${cat?.tone}`}>
                      {cat?.label.slice(0, 1)}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 text-sm">
                        <span className="font-semibold text-foreground">{r.students?.full_name ?? "Student"}</span>
                        <span className={`text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded ${cat?.tone}`}>{cat?.label}</span>
                        {r.visible_to_parent && <span className="text-[10px] text-muted-foreground">· Visible to parent</span>}
                      </div>
                    ...
                    <div className="shrink-0 flex items-center gap-2">
                      <WhatsAppBroadcast
                        ...
                        recipients={[{
                          id: r.student_id,
                          name: r.students?.parent_name || `${r.students?.full_name ?? "Student"}'s parent`,
            ...
            {totalCount > 0 && (
              <div className="flex items-center justify-between p-4 bg-card border border-border rounded-xl shadow-xs text-card-foreground">
                <p className="text-xs text-muted-foreground">
                  Showing {page * pageSize + 1} to {Math.min((page + 1) * pageSize, totalCount)} of {totalCount} remarks
                </p>
                <div className="flex gap-2">
                  <button
                    disabled={page === 0}
      ...
      {open && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50 text-foreground" onClick={() => setOpen(false)}>
          <form onClick={(e) => e.stopPropagation()} onSubmit={submit} className="bg-card rounded-xl p-6 w-full max-w-md space-y-4 shadow-lg text-card-foreground">
            ...
            <div>
              ...
              <select required value={studentId} onChange={(e) => setStudentId(e.target.value)} className="mt-1 w-full px-3 py-2 text-sm border border-border rounded-md bg-background text-foreground focus:outline-none">
            ...
            <div>
              ...
              <textarea required value={content} onChange={(e) => setContent(e.target.value)} rows={4} placeholder="Showed great improvement in fractions today." className="mt-1 w-full px-3 py-2 text-sm border border-border rounded-md bg-background text-foreground focus:outline-none" />
            </div>
            <div className="flex gap-2 justify-end">
              <button type="button" onClick={() => setOpen(false)} className="px-4 py-2 text-sm font-medium border border-border rounded-md hover:bg-secondary cursor-pointer">Cancel</button>
              <button type="submit" disabled={busy} className="px-4 py-2 text-sm font-semibold bg-primary text-primary-foreground rounded-md disabled:opacity-50 cursor-pointer">{busy ? "Saving…" : "Add Remark"}</button>
            </div>
          </form>
        </div>
      )}
