...
import { Bell, CheckCheck, BookOpen, CalendarCheck, MessageSquare, Megaphone, Wallet } from "lucide-react";
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          unread > 0 ? (
            <button onClick={markAllRead} className="px-3 py-1.5 text-sm border border-border rounded-md inline-flex items-center gap-1">
      ...
      <div className="flex-1 overflow-y-auto p-8">
        {items.length === 0 ? (
          <div className="bg-card border border-dashed border-border rounded-xl p-16 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">New homework, remarks, fees and announcements appear here in real time.</p>
          </div>
        ) : (
          <div className="bg-card border border-border rounded-xl divide-y divide-border">
            {items.map((n) => (
              <div key={n.id} onClick={() => markRead(n.id)} className={`p-4 flex gap-4 cursor-pointer hover:bg-secondary/40 ${!n.read_at ? "bg-brand/5" : ""}`}>
