...
import { Sparkles, CalendarCheck, BookOpen, MessageSquare, Megaphone, Download, AlertCircle, Trophy, Award, Share2, User } from "lucide-react";
...
interface HomeworkRow { id: string; title: string; subject: string | null; due_date: string | null; file_url: string | null; file_type: string }
interface RemarkRow { id: string; category: string; content: string; created_at: string }
...
function ParentDashboard() {
  ...
  // Achievements states
  const [childRank, setChildRank] = useState<{ rank_position: number; percentage: number; gpa: number; academic_year: string } | null>(null);
  const [childAwards, setChildAwards] = useState<Array<{ id: string; category: string; title: string; description: string; issued_at: string }>>([]);
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        const list = ((data ?? []) as unknown as Array<Child & { classes: { name: string } | { name: string }[] | null; schools: { status: string } | { status: string }[] | null }>).map((c) => ({
          ...c, 
  ...
  useEffect(() => {
    ...
    (async () => {
      const [att, hw, rem, ann, rnk, awd] = await Promise.all([
        supabase.from("attendance").select("date, status").eq("student_id", child.id).gte("date", monthAgo).order("date", { ascending: false }),
        child.class_id
          ? supabase.from("homework").select("id, title, subject, due_date, file_url, file_type").eq("class_id", child.class_id).order("created_at", { ascending: false }).limit(10)
          : Promise.resolve({ data: [] as HomeworkRow[] }),
        supabase.from("remarks").select("id, category, content, created_at").eq("student_id", child.id).order("created_at", { ascending: false }).limit(10),
        ...
        supabase.from("rankings").select("rank_position, percentage, gpa, academic_year").eq("student_id", child.id).eq("is_published", true).maybeSingle(),
        supabase.from("awards").select("id, category, title, description, issued_at").eq("student_id", child.id).eq("is_published", true).order("issued_at", { ascending: false })
      ]);
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          children.length > 1 ? (
            <select value={selected} onChange={(e) => setSelected(e.target.value)} className="px-3 py-1.5 text-sm border border-border rounded-md bg-card">
        ...
        {children.length === 0 ? (
          <div className="bg-card border border-dashed border-border rounded-xl p-16 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-2 max-w-md mx-auto">
              Ask your child's school admin to add your email <span className="font-mono">{profile?.email}</span> as the parent email for your child in their school's system. You'll be linked automatically.
            </p>
          </div>
          ...
          <div className="bg-card border border-border rounded-xl p-12 text-center max-w-2xl mx-auto shadow-sm">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Access to the digital portal for <strong>{child.full_name}</strong>'s school has been temporarily suspended.
            </p>
          ...
          <>
            ...
            <section className="bg-primary text-primary-foreground rounded-2xl p-8 relative overflow-hidden">
              ...
              <div className="relative">
                <div className="inline-flex items-center gap-2 text-xs font-medium uppercase tracking-wider text-brand-foreground bg-brand/30 px-3 py-1 rounded-full">
                  <Sparkles className="size-3.5" /> Today's Digest · {new Date().toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric" })}
                </div>
                <h2 className="text-3xl font-bold mt-4">How {child?.full_name.split(" ")[0]}'s day went</h2>
                <p className="text-sm opacity-80 mt-2">A daily summary your child's school sends every evening. Everything that happened — at a glance.</p>

                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-6">
                  <DigestCell label="Attendance" value={todayAtt ? todayAtt.status.replace("_", " ") : "Not marked"} />
                  <DigestCell label="Homework today" value={`${homework.filter((h) => h.due_date && new Date(h.due_date) >= new Date(new Date().toDateString())).length}`} />
                  <DigestCell label="New remarks" value={`${remarks.filter((r) => new Date(r.created_at).toDateString() === new Date().toDateString()).length}`} />
            ...
            <section className="bg-white border border-border rounded-2xl p-6 shadow-xs space-y-4 text-foreground">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Officially published certifications and dashboard achievements.</p>
                </div>
                {childRank && (
                  <span className="bg-amber-50 text-amber-700 font-extrabold border border-amber-200 text-xs px-3.5 py-1.5 rounded-full flex items-center gap-1.5 shadow-xs">
                    <Sparkles className="size-3.5 text-amber-500 animate-pulse" /> Rank Position: #{childRank.rank_position} ({childRank.percentage}%)
                  </span>
                )}
              </div>
              ...
              {childAwards.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {childAwards.map(aw => (
                    <div key={aw.id} className="p-4 border border-slate-100 rounded-xl bg-slate-50/40 flex flex-col justify-between hover:border-slate-200 transition-colors">
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          <span className="bg-brand/10 text-brand text-[9px] uppercase font-black px-2 py-0.5 rounded">
                            {badgeLabel(aw.category)}
                          </span>
                          <span className="text-[10px] text-muted-foreground">{new Date(aw.issued_at).toLocaleDateString()}</span>
                        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 bg-card border border-border rounded-xl p-6">
                <h3 className="font-semibold flex items-center gap-2"><BookOpen className="size-4 text-brand" /> Recent Homework</h3>
                {homework.length === 0 ? (
                  ...
                  <div className="space-y-3 mt-4">
                    {homework.map((h) => (
                      <div key={h.id} className="flex items-center justify-between p-3 rounded-lg border border-border">
                        ...
                        {h.file_url && (
                          <a href={h.file_url} target="_blank" rel="noreferrer" className="text-xs text-brand font-medium inline-flex items-center gap-1">
              ...
              <div className="bg-card border border-border rounded-xl p-6">
                <h3 className="font-semibold flex items-center gap-2"><MessageSquare className="size-4 text-brand" /> Teacher Remarks</h3>
                {remarks.length === 0 ? (
                  ...
                  <div className="space-y-3 mt-4">
                    {remarks.map((r) => (
                      <div key={r.id} className="border-l-2 border-brand pl-3">
                        <p className="text-[10px] uppercase tracking-wider font-bold text-brand">{r.category}</p>
              ...
              {/* Attendance history */}
              <div className="bg-card border border-border rounded-xl p-6">
                <h3 className="font-semibold flex items-center gap-2"><CalendarCheck className="size-4 text-brand" /> Last 30 Days</h3>
                <div className="mt-4 flex flex-wrap gap-1">
                  {attendance.slice(0, 30).reverse().map((a) => (
                    <div key={a.date} title={`${a.date} · ${a.status}`}
                      className={`size-5 rounded ${
                        a.status === "present" ? "bg-success" :
                        a.status === "absent" ? "bg-danger" :
                        a.status === "late" ? "bg-warning" : "bg-brand"
                      }`} />
                  ))}
                </div>
                <p className="text-xs text-muted-foreground mt-3">{last30Present} present out of {attendance.length} marked days</p>
              </div>

              {/* Announcements */}
              <div className="lg:col-span-2 bg-card border border-border rounded-xl p-6">
                <h3 className="font-semibold flex items-center gap-2"><Megaphone className="size-4 text-brand" /> School Announcements</h3>
          ...
          >
            ...
            <div className="flex flex-col items-center text-center my-6 space-y-4">
              <div className="size-24 rounded-full border-4 border-black/10 bg-white/40 text-slate-700 flex items-center justify-center font-bold text-3xl shadow-lg relative overflow-hidden">
                {child?.photo_url ? (
                  <img src={child.photo_url} alt="" className="size-full object-cover" crossOrigin="anonymous" />
                ) : (
                  child?.full_name?.slice(0, 1) || "S"
                )}
              </div>
              <div className="space-y-1">
                <p className="text-[10px] uppercase font-bold tracking-widest opacity-70">Honor Roll Achievement</p>
              ...
              <div className="px-5 py-3 bg-white/40 border border-black/5 rounded-2xl text-center shadow-md space-y-1 max-w-[200px]">
                ...
                <div className="text-[9px] uppercase tracking-wider opacity-85 font-bold">Verified Honor</div>
                {childRank && (
                  <div className="text-[10px] font-black text-brand">GPA {childRank.gpa} · Percentage {childRank.percentage}%</div>
                )}
              </div>
            </div>
            ...
            <div className="flex items-end justify-between border-t border-black/10 pt-4 text-left">
              <div className="space-y-1">
                <span className="text-[8px] uppercase tracking-wider font-bold opacity-60">Verification Code</span>
              ...
              <div className="text-right space-y-2">
                <span className="font-serif italic text-amber-600 font-bold text-sm tracking-wide block">Nirosha Reddy</span>
        ...
        {selectedAward && (
          ...
          >
            ...
            <div className="space-y-4 my-4 flex flex-col items-center">
              <h3 className="font-serif text-2xl font-extrabold text-amber-800 tracking-tight">CERTIFICATE OF EXCELLENCE</h3>
              ...
              <p className="text-[10px] italic text-slate-600">This certificate is proudly presented to</p>
              ...
              <p className="text-xs text-slate-600 max-w-lg mx-auto leading-relaxed">
                Awarded for outstanding achievements in <span className="font-bold text-amber-800">{selectedAward.title}</span>. Demonstrated academic dedication, good character, and excellence in school values for the term.
              </p>
            </div>
            ...
            <div className="w-full flex items-end justify-between px-10 border-t border-slate-100 pt-6">
              <div className="text-center space-y-1.5 w-32 text-xs">
                <span className="font-serif italic text-amber-700 font-semibold text-xs tracking-wide">Nirosha Reddy</span>
              ...
              <div className="text-center space-y-1.5 w-32 text-xs">
                <span className="font-bold text-slate-700">{new Date(selectedAward.issued_at).toLocaleDateString()}</span>
                <div className="h-[1px] bg-slate-300 w-full" />
