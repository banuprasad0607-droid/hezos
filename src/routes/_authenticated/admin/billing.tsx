...
function BillingPage() {
  ...
  const loadData = async () => {
    ...
    const [sc, sb, inv, st, tr] = await Promise.all([
      supabase.from("schools").select("name, address, student_limit, teacher_limit").eq("id", schoolId!).single(),
      supabase.from("subscriptions").select("plan, status, monthly_amount, billing_cycle, current_period_end").eq("school_id", schoolId!).single(),
      supabase.from("platform_invoices").select("*").eq("school_id", schoolId!).order("created_at", { ascending: false }),
  ...
  const handleDownloadInvoice = async (invoice: Invoice) => {
    try {
      ...
      const canvas = await safeHtml2Canvas(container.firstElementChild as HTMLElement, { scale: 2 });
  ...
  if (loading || !sub || !school) return <div className="p-8 text-center">Loading billing data...</div>;
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className={`text-xs px-2 py-0.5 rounded uppercase font-semibold tracking-wider ${sub.status === 'active' ? 'bg-brand-soft text-brand' : 'bg-warning/20 text-warning-dark'}`}>
                  {sub.status}
                </span>
              </div>
              <p className="text-muted-foreground text-sm">
                You are currently on the {sub.plan} tier billed {sub.billing_cycle}.
                {sub.current_period_end && ` Your current billing period ends on ${new Date(sub.current_period_end).toLocaleDateString()}.`}
              </p>
            </div>
            <div className="text-right">
              <div className="text-3xl font-bold">${sub.monthly_amount}</div>
              <div className="text-xs text-muted-foreground uppercase tracking-wider">Per {sub.billing_cycle === 'yearly' ? 'Year' : sub.billing_cycle === 'quarterly' ? 'Quarter' : 'Month'}</div>
            </div>
          </div>
          ...
          <div className="mt-8 grid grid-cols-2 gap-8">
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="text-muted-foreground">{counts.students} / {school.student_limit}</span>
              </div>
            ...
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="text-muted-foreground">{counts.teachers} / {school.teacher_limit}</span>
              </div>
              <div className="h-2 bg-muted rounded-full overflow-hidden">
                ...
                <tbody className="divide-y divide-border">
                  {invoices.map((inv) => (
                    <tr key={inv.id} className="hover:bg-muted/30">
                      ...
                      <td className="px-6 py-3 text-muted-foreground">{new Date(inv.created_at).toLocaleDateString()}</td>
                      ...
                      <td className="px-6 py-3">
                        <span className={`inline-flex items-center gap-1 text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded ${
                          inv.status === 'paid' ? 'bg-success/20 text-success-dark' : 'bg-warning/20 text-warning-dark'
                        }`}>
                      ...
                      <td className="px-6 py-3 text-right">
                        ...
                        >
                          {generatingPdf === inv.id ? "Generating..." : <><Download className="size-3" /> PDF</>}
                        </button>
                      </td>
                    </tr>
                  ))}
