...
function DashboardPage() {
  const { currentSchoolId: effectiveSchoolId, profile, roles, loading: tenantLoading, error: tenantError } = useTenant();
  usePageTitle("Dashboard");
  const [stats, setStats] = useState<Stats | null>(null);
  const [remarks, setRemarks] = useState<Remark[]>([]);

  const isParent = roles.includes("parent") && !roles.includes("admin") && !roles.includes("teacher");
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [students, teacherRoles, attRows, hwToday, hwPending, recentRemarks] =
        await Promise.all([
          ...
          supabase.from("user_roles").select("user_id", { count: "exact", head: true }).eq("school_id", effectiveSchoolId).eq("role", "teacher"),
          supabase.from("attendance").select("status").eq("school_id", effectiveSchoolId).eq("date", today),
          supabase.from("homework").select("id", { count: "exact", head: true }).eq("school_id", effectiveSchoolId).gte("created_at", today),
          supabase.from("homework").select("id", { count: "exact", head: true }).eq("school_id", effectiveSchoolId).gte("due_date", today),
          supabase.from("remarks").select("id, category, content, created_at, students(full_name)").eq("school_id", effectiveSchoolId).order("created_at", { ascending: false }).limit(5),
        ]);

      if (!mounted) return;
      const present = (attRows.data ?? []).filter((r) => r.status === "present" || r.status === "late" || r.status === "half_day").length;
      setStats({
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "attendance", filter: `school_id=eq.${effectiveSchoolId}` }, () => void load())
      .on("postgres_changes", { event: "*", schema: "public", table: "remarks", filter: `school_id=eq.${effectiveSchoolId}` }, () => void load())
      .subscribe();
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-8 bg-[#F9FAFB] min-h-screen">
        <div className="max-w-md text-center p-6 bg-white rounded-2xl shadow-sm border border-slate-100">
          ...
          <p className="text-sm text-muted-foreground mb-4">Please contact the administrator or check your connection.</p>
          <button onClick={() => window.location.reload()} className="bg-primary text-primary-foreground px-4 py-2 rounded-md text-sm font-medium hover:opacity-90">
            Retry
          </button>
        </div>
      </div>
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-8 bg-[#F9FAFB] min-h-screen">
        <div className="max-w-md text-center p-6 bg-white rounded-2xl shadow-sm border border-slate-100">
          ...
          <p className="text-sm text-muted-foreground mb-4">You are not associated with any school. Please contact the administrator.</p>
        </div>
      </div>
    );
  }
  ...
  if (isParent) {
    return (
      <div className="p-8">
        ...
        <Link to="/parent" className="mt-4 inline-block text-brand font-medium">Open Parent Dashboard →</Link>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#F9FAFB] p-8 space-y-6 text-foreground">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Welcome back, {profile?.full_name?.split(" ")[0] || "Admin"}</h1>
          <p className="text-muted-foreground text-sm mt-1">
            {roles.includes("admin") ? "Admin Dashboard" : "Teacher Dashboard"} • School Management & Academics
          </p>
        </div>
        <Link 
          to="/students" 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <KpiCard 
          ...
          subtext={stats?.attendanceToday.total ? `of ${stats.attendanceToday.total} marked` : "Not marked yet"}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
          ...
          <p className="text-sm text-muted-foreground mb-6">Review and acknowledge student remarks</p>
          
          <div className="space-y-4">
            {remarks.length === 0 ? (
              <div className="text-sm text-muted-foreground text-center py-4">No recent remarks.</div>
            ) : (
              remarks.map((r) => (
                <div key={r.id} className="flex items-center justify-between p-4 rounded-xl border border-slate-100 bg-slate-50/50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-sm">{r.student?.full_name || "Unknown Student"}</p>
                      <p className="text-xs text-muted-foreground capitalize mt-0.5">{r.category} • {r.content.length > 30 ? r.content.slice(0, 30) + '...' : r.content}</p>
                    </div>
                  </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
          <div className="flex items-center justify-between mb-2">
            ...
            <Link to="/attendance" className="text-sm font-medium flex items-center gap-1 hover:text-brand transition-colors">
            ...
            <div>
              <div className="flex justify-between text-sm mb-2">
                ...
                <span className="font-semibold text-brand">
                  {stats?.attendanceToday.total ? Math.round((stats.attendanceToday.present / stats.attendanceToday.total) * 100) : 0}%
                </span>
              </div>
              <div className="h-2.5 w-full bg-slate-100 rounded-full overflow-hidden">
                <div 
                  className="h-full bg-brand rounded-full transition-all duration-500"
                  style={{ width: `${stats?.attendanceToday.total ? (stats.attendanceToday.present / stats.attendanceToday.total) * 100 : 0}%` }}
                />
              </div>
            </div>
...
function KpiCard({ 
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex items-center justify-between">
      <div>
        <p className="text-xs font-semibold text-muted-foreground mb-1 tracking-wider uppercase">{label}</p>
        <div className="flex items-end gap-2">
          <h3 className="text-3xl font-bold text-foreground">{value}</h3>
          {subtext && (
            <span className={`text-xs font-medium mb-1 ${subtextColor}`}>
              {subtext}
            </span>
          )}
        </div>
