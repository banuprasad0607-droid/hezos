    ...
    var arrayToStringHelper = {
      ...
      applyCanBeUsed: {
        ...
        uint8array: (function () {
          try {
            return (
              support2.uint8array && String.fromCharCode.apply(null, new Uint8Array(1)).length === 1
            );
          } catch (e) {
            return false;
          }
        ...
        nodebuffer: (function () {
          try {
            return (
              support2.nodebuffer &&
              String.fromCharCode.apply(null, nodejsUtils2.allocBuffer(1)).length === 1
            );
          } catch (e) {
            return false;
          }
      ...
      var promise = external2.Promise.resolve(inputData).then(function (data) {
        var isBlob =
          ...
          (data instanceof Blob ||
            ["[object File]", "[object Blob]"].indexOf(Object.prototype.toString.call(data)) !==
              -1);
...
function requireCompressedObject() {
  ...
  CompressedObject.prototype = {
    ...
    getContentWorker: function () {
      var worker = new DataWorker(external2.Promise.resolve(this.compressedContent))
    ...
    getCompressedWorker: function () {
      return new DataWorker(external2.Promise.resolve(this.compressedContent))
...
function requireZipFileWorker() {
  ...
  var generateZipParts = function (
    ...
    var file = streamInfo["file"],
      ...
      useCustomEncoding = encodeFileName !== utf82.utf8encode,
      encodedFileName = utils2.transformTo("string", encodeFileName(file.name)),
      utfEncodedFileName = utils2.transformTo("string", utf82.utf8encode(file.name)),
      comment = file.comment,
      encodedComment = utils2.transformTo("string", encodeFileName(comment)),
      utfEncodedComment = utils2.transformTo("string", utf82.utf8encode(comment)),
    ...
    var dirRecord =
      signature2.CENTRAL_FILE_HEADER + // version made by (00: DOS)
  ...
  var generateCentralDirectoryEnd = function (
    ...
    dirEnd =
      signature2.CENTRAL_DIRECTORY_END + // number of this disk
  ...
  var generateDataDescriptors = function (streamInfo) {
    var descriptor = "";
    descriptor =
      signature2.DATA_DESCRIPTOR + // crc-32                          4 bytes
...
function requireObject() {
  ...
  var fileAdd = function (name, data, originalOptions) {
    var dataType = utils2.getTypeOf(data),
      parent;
    ...
    if (data instanceof CompressedObject || data instanceof GenericWorker) {
      ...
      zipObjectContent = utils2.prepareContent(
        name,
        data,
        o.binary,
      ...
      if (compression === null) {
        throw new Error(
          "Corrupted zip : compression " +
            utils2.pretty(this.compressionMethod) +
            " unknown (inner file : " +
            utils2.transformTo("string", this.fileName) +
            ")",
        );
      }
...
function requireZipEntries() {
  ...
  ZipEntries.prototype = {
      ...
      if (!this.reader.readAndCheckSignature(expectedSignature)) {
        ...
        throw new Error(
          "Corrupted zip or bug: unexpected signature (" +
            utils2.pretty(signature2) +
            ", expected " +
            utils2.pretty(expectedSignature) +
            ")",
        );
      }
    },
    ...
    readEndOfCentral: function () {
      ...
      if (
        this.diskNumber === utils2.MAX_VALUE_16BITS ||
        this.diskWithCentralDirStart === utils2.MAX_VALUE_16BITS ||
        this.centralDirRecordsOnThisDisk === utils2.MAX_VALUE_16BITS ||
        this.centralDirRecords === utils2.MAX_VALUE_16BITS ||
        this.centralDirSize === utils2.MAX_VALUE_32BITS ||
        this.centralDirOffset === utils2.MAX_VALUE_32BITS
      ) {
        this.zip64 = true;
